...
fn fn_body(results: &mut Vec<TextEdit>, e2e_test: &InkE2ETest) {
    ...
    process_block_expr(results, &body);
}

fn process_block_expr(results: &mut Vec<TextEdit>, block_expr: &ast::BlockExpr) {
    ...
    for stmt in stmt_list.statements() {
        ...
        loop {
            ...
            while let Some(expr) = top_expr.take().or(prev_sub_exprs_iter.next()) {
                process_expr(results, &mut curr_sub_exprs, &expr);
            }
...
fn process_expr(
    ...
    curr_sub_exprs: &mut Vec<ast::Expr>,
    expr: &ast::Expr,
) {
    ...
    match expr {
        ...
        ast::Expr::FormatArgsExpr(expr) => {
            if let Some(template) = expr.template() {
                curr_sub_exprs.push(template);
            }
            curr_sub_exprs.extend(expr.args().filter_map(|arg| arg.expr()));
        }
        ast::Expr::IfExpr(expr) => {
            process_if_expr(results, &expr);
        }
        ast::Expr::IndexExpr(expr) => {
            if let Some(base) = expr.base() {
                curr_sub_exprs.push(base);
            }
            if let Some(index) = expr.index() {
                curr_sub_exprs.push(index);
            }
        }
        ...
        ast::Expr::AwaitExpr(expr) => collect_sub_expr!(expr, curr_sub_exprs),
        ast::Expr::BreakExpr(expr) => collect_sub_expr!(expr, curr_sub_exprs),
        ast::Expr::CastExpr(expr) => collect_sub_expr!(expr, curr_sub_exprs),
        ast::Expr::FieldExpr(expr) => collect_sub_expr!(expr, curr_sub_exprs),
        ast::Expr::LetExpr(expr) => collect_sub_expr!(expr, curr_sub_exprs),
        ast::Expr::ParenExpr(expr) => collect_sub_expr!(expr, curr_sub_exprs),
        ast::Expr::PrefixExpr(expr) => collect_sub_expr!(expr, curr_sub_exprs),
        ast::Expr::RefExpr(expr) => collect_sub_expr!(expr, curr_sub_exprs),
        ast::Expr::ReturnExpr(expr) => collect_sub_expr!(expr, curr_sub_exprs),
        ast::Expr::TryExpr(expr) => collect_sub_expr!(expr, curr_sub_exprs),
        ast::Expr::YieldExpr(expr) => collect_sub_expr!(expr, curr_sub_exprs),
        ast::Expr::YeetExpr(expr) => collect_sub_expr!(expr, curr_sub_exprs),
...
fn process_if_expr(results: &mut Vec<TextEdit>, if_expr: &ast::IfExpr) {
    if let Some(expr) = if_expr.then_branch() {
        process_block_expr(results, &expr);
    }
    if let Some(branch) = if_expr.else_branch() {
        match branch {
            ast::ElseBranch::Block(expr) => process_block_expr(results, &expr),
            ast::ElseBranch::IfExpr(expr) => process_if_expr(results, &expr),
        }
    }
}
