                                ...
                                .flat_map(|arg_kind| {
                                        ...
                                        || *arg_kind == InkArgKind::Namespace
                                    {
                                        ...
                                        let primary_args: Vec<InkArgKind> = [*arg_kind]
                                            ...
                                            .filter(|arg_kind| {
                                                arg_kind.is_entity_type()
                                                    && possible_args.contains(arg_kind)
                                            })
                                            .collect();
...
fn add_signature(
    ...
    for arg_kind in args.iter().sorted() {
        ...
        let doc = [&arg_value_kind.detail(), arg_kind.detail(version)]
            .iter()
...
fn anchor_signature(
    ...
    let args: Vec<InkArgKind> = [*anchor_arg]
        ...
        .collect();
    add_signature(results, &args, focused_arg, range, version);
}
...
fn complementary_signature(
    ...
    if *arg_kind == InkArgKind::Namespace {
        add_signature(results, &[*arg_kind], focused_arg, range, version);
    }
    ...
    fn signature_help_works() {
        ...
        let fixtures_v5_only = vec![
            ...
            (
                ...
                Some("ink("),
                vec![(
                    "function: u16, handle_status: bool",
                    (Some("("), Some("<-)")),
                    vec![
                        (Some("<-function"), Some("u16")),
        ...
        for (version, fixtures) in [
            ...
            (
                ...
                [
                    (
                        ...
                        Some("ink("),
                        vec![(
                            "constructor, default, name: &str, payable, selector: u32 | _",
                            (Some("("), Some("<-)")),
                            vec![
                                (Some("<-constructor"), Some("constructor")),
                                (Some("<-default"), Some("default")),
                    ...
                    (
                        ...
                        Some("ink("),
                        vec![(
                            "message, default, name: &str, payable, selector: u32 | _ | @",
                            (Some("("), Some("<-)")),
                            vec![
                                (Some("<-message"), Some("message")),
                                (Some("<-default"), Some("default")),
                    ...
                    (
                        ...
                        Some("ink("),
                        vec![(
                            "event, anonymous, name: &str, signature_topic: &str",
                            (Some("("), Some("<-)")),
                            vec![
                                (Some("<-event"), Some("event")),
                                (Some("<-anonymous"), Some("anonymous")),
                                (Some("<-name"), Some("name: &str")),
                                (Some("<-signature_topic"), Some("signature_topic: &str")),
                            ],
                            0,
                        )],
            ...
            .chain(fixtures)
            {
                ...
                let results = signature_help(&InkFile::parse(code), offset, version);
